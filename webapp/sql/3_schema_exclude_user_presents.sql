...
CREATE TABLE `user_decks` (
  `id` VARCHAR(15) NOT NULL,
  `user_id` VARCHAR(15) NOT NULL comment 'ユーザID', 
...
CREATE TABLE `login_bonus_reward_masters` (
  `id` VARCHAR(15) NOT NULL,
  `login_bonus_id` VARCHAR(15) NOT NULL comment 'ログインボーナスID',
...
CREATE TABLE `user_present_all_received_history` (
  `id` VARCHAR(15) NOT NULL,
  `user_id` VARCHAR(15) NOT NULL comment '受けとったユーザID',
  `present_all_id` VARCHAR(15) NOT NULL comment '全員プレゼントマスタのID',
  `received_at` bigint NOT NULL comment '受け取った日時',
  `created_at` bigint NOT NULL,
  `updated_at`bigint NOT NULL,
  `deleted_at` bigint default NULL,
  PRIMARY KEY (`id`),
  INDEX idx_user_id_present_all_id (`user_id`, `present_all_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE `gacha_item_masters` (
  `id` VARCHAR(15) NOT NULL,
  `gacha_id` VARCHAR(15) NOT NULL comment 'ガチャ台のID',
  `item_type` int(1) NOT NULL comment 'アイテム種別',
  `item_id` int NOT NULL comment 'アイテムID',
  `amount` int NOT NULL comment 'アイテム数',
